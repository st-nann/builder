...
const builder: any = document.querySelectorAll(
  "div.builder-template-container"
);
const Builder = () => {
  _.forEach(builder, (node: any) => {
    ...
    const personalizeBaseUrl = node.getAttributeNode(
      "data-personalize-baseurl"
    );
    ...
    const propTemplateJson =
      node && !_.isNull(template) ? JSON.parse(template.value) : undefined;
      ...
      const vue = new Vue({
        ...
        render: (h) =>
          h(BuilderTemplate, {
            ...
            on: {
              change(template: any) {
                (vue.$el as any).dataset.resultTemplate = JSON.stringify(
                  template
                );
              },
            },
